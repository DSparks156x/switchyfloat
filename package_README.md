...
For more details, read the [1.2 release post](https://pev.dev/t/lefloat-version-1-2/2795).

## Installation
### Upgrading
Back up your package config (either by **Backup Configs** on the Start page, or by saving the XML on **Lefloat Cfg**).

If upgrading from Lefloat 1.1 to 1.2, an automatic config restore dialog should pop up. Confirm it to restore. This is the preferred way to update the config from now on, as it will also migrate any changed options to the new version. Other methods of restoring the config will not do that.
...
On 6.05+ firmware, the package should ride well without the need to configure anything in Lefloat Cfg. On 6.02, the **Low and High Tiltback voltages** on the **Specs** tab of **Lefloat Cfg** still need to be set according to your battery specs.
