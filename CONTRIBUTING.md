# Contributing
Bugfix Pull Requests are always welcome. If you want to implement a feature, it is advised get in touch first, to make sure your feature is in line with the direction Lefloat development is heading.
...
Date:   Sun Sep 17 13:35:16 2023 +0200
    ...
    Feature: Separate Mahony KP configuration between package and firmware
     ...
     (firmware) and the Lefloat Config (package). The App Config KP is now
     ...
     happens, Lefloat will set the following values in App Config -> IMU if
     ...
     >
     A new Lefloat Config item Accelerometer Confidence Decay has been
     added to the package, which is used for the Lefloat balance filter.
```
...
## Git Hooks
Lefloat uses `pre-commit` for managing git hooks. Again, you need to install this dependency in your development environment. Then run this to install the commit hook in the repo:
```sh
pre-commit install
```
